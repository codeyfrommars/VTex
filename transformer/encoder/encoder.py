...
class Encoder(nn.Module):
    ...
    def __init__(self, growth_rate, block_depth, compression, dropout, height, width, dim_model, device):
        ...
        self.embed = EncoderEmbedding(height//32, width//32, dim_model) # /32 if using pretrained densenet
    ...
    def forward(self, img):
        ...
        # print(features.size())
        assert (features.size() == (batch_size, height//32, width//32, self.dim_model)), "feature reshape incorrect shape"
        ...
        assert (features.size() == (batch_size, (height//32) * (width//32), self.dim_model)), "Encoder output incorrect shape"
