...
for training_inkml in training_inkmls:
    ...
    for index, trace_grp in enumerate(trace_groups):
        ...
        try:
            ...
            pattern = {'features': image, 'label': label}
            im = Image.fromarray(image)
            imageName = ("train_"+str(index)+"_"+str(label.replace("\\", "bslash").replace("/", "fslash"))+".png")
            print(imageName)
            im.save(os.path.join(train_out_dir, imageName))
...
for testing_inkml in testing_inkmls:
    ...
    for index, trace_grp in enumerate(trace_groups):
        ...
        try:
            ...
            #pattern = {'features': image.flatten(), 'label': label}
            pattern = {'features': image, 'label': label}
            imageName = ("test_"+str(index)+"_"+str(label.replace("\\", "bslash").replace("/", "fslash"))+".png")
            print(imageName)
            im.save(os.path.join(test_out_dir, imageName))
            #test.append(pattern)
        except Exception as e:
            print(e)
            # Ignore damaged trace groups
